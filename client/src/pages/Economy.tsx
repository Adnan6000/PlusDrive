...
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'; // ✅ Added TanStack Query
...
export default function Economy() {
  ...
  const [invDueDate, setInvDueDate] = useState(new Date().toLocaleDateString('en-CA'));  
  const [selectedInvoice, setSelectedInvoice] = useState<any>(null);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);

  // ✅ 1. Optimized Invoice Fetching
  const { data: invoices = [] } = useQuery({
    queryKey: ['invoices', user.id, isInstructor],
    queryFn: async () => {
  ...
  // ✅ 2. Optimized Student List Fetching
  const { data: students = [] } = useQuery({
    ...
    staleTime: 1000 * 60 * 10, // Student lists change rarely
  });

  // ✅ 3. Mutation for Generating Invoices
  const generateInvoiceMutation = useMutation({
    mutationFn: (newInvoice: any) => api.post('/booking/generate-invoice', newInvoice),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['invoices'] });
      alert("Invoice(s) generated!");
    },
    onError: () => alert("Generation failed. Check console.")
  });
  ...
  const formatDate = (dateString: string) => {
    ...
    const date = new Date(dateString.split('T')[0]); // ✅ Fixed timezone shift
              ...
              {selectedInvoice.proofUrl ? (
                <div className="space-y-4">
                  <div className="border rounded-lg overflow-hidden bg-white max-w-sm">
                    <img src={selectedInvoice.proofUrl} alt="Payment Proof" className="w-full h-auto cursor-pointer" onClick={() => window.open(selectedInvoice.proofUrl, '_blank')}/>
                  </div>
                  <button onClick={() => window.open(selectedInvoice.proofUrl, '_blank')} className="text-blue-600 text-xs font-bold flex items-center gap-1" title="View receipt in full screen"><FaExternalLinkAlt /> View Full Image</button>
                  {selectedInvoice.status === 'REVIEWING' && (
                    <div className="flex gap-4 pt-4 border-t">
                      <button onClick={() => handleInstructorDecision(selectedInvoice.id, 'PAID')} className="bg-green-600 text-white px-6 py-2 rounded-full font-bold text-xs flex items-center gap-2" title="Approve this payment"><FaCheckCircle /> Approve Payment</button>
                      <button onClick={() => handleInstructorDecision(selectedInvoice.id, 'REJECTED')} className="bg-red-600 text-white px-6 py-2 rounded-full font-bold text-xs flex items-center gap-2" title="Reject this payment"><FaTimes /> Reject Proof</button>
                    </div>
                  )}
                </div>
            ...
            selectedInvoice.status === 'PENDING' && (
              <div>
                ...
                <div className="flex items-center gap-6">
                  <input id="file-upload" type="file" title="Select payment proof image" className="text-xs" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} />
                  {selectedFile && <button onClick={() => handleUploadProof(selectedInvoice.id)} className="bg-blue-600 text-white px-8 py-2 rounded-full font-bold text-xs flex items-center gap-2" title="Submit receipt"><FaCheckCircle /> Submit Receipt</button>}
                </div>
              </div>
            )
          )}
          <div className="mt-4 flex justify-between items-center">
             <span className={`text-xs font-bold uppercase ${selectedInvoice.status === 'PAID' ? 'text-green-600' : 'text-orange-600'}`}>Status: {selectedInvoice.status}</span>
  ...
  return (
    <div className="space-y-6 p-4">
      {isInstructor && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="font-bold text-slate-700 mb-6 flex items-center gap-2"><FaFileInvoiceDollar /> Create Invoice</h3>
            <div className="space-y-4">
              <div className="relative">
                <FaSearch className="absolute left-3 top-3 text-slate-300" />
                <input title="Search for student" className="w-full border pl-10 p-2 rounded-lg text-sm" placeholder="Search by name/email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>
              <select title="Choose a student" disabled={applyToAll} className="w-full border p-2 rounded-lg text-sm bg-white" value={invStudentId} onChange={e => setInvStudentId(e.target.value)}>
                <option value="">-- {filteredStudents.length} students found --</option>
                {filteredStudents.map((s: any) => <option key={s.id} value={s.id}>{s.fullName}</option>)}
              </select>
              <div className="flex items-center gap-2">
                <input id="all-check" type="checkbox" checked={applyToAll} onChange={e => setApplyToAll(e.target.checked)} />
              ...
              <textarea title="Description" className="w-full border p-2 rounded-lg text-sm h-20" placeholder="Description..." value={invDesc} onChange={e => setInvDesc(e.target.value)} />
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Price DKK (Excl. VAT)</label>
                  <input title="Base price" type="number" className="w-full border p-2 rounded-lg text-sm" value={invPrice} onChange={e => setInvPrice(e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Due Date</label>
                  <input title="Due date" type="date" className="w-full border p-2 rounded-lg text-sm" value={invDueDate} onChange={e => setInvDueDate(e.target.value)} />
                </div>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit">
            <h4 className="font-bold mb-4 flex items-center gap-2"><FaUniversity /> Bank Settings</h4>
            <div className="space-y-3">
              <label className="text-[10px] font-bold text-gray-400 block uppercase">Reg.nr.</label>
              <input title="Reg number" className="w-full border p-2 text-sm rounded" value={bankInfo.bankRegNum} onChange={e => setBankInfo({...bankInfo, bankRegNum: e.target.value})} />
              <label className="text-[10px] font-bold text-gray-400 block uppercase">Kontonr.</label>
              <input title="Account number" className="w-full border p-2 text-sm rounded" value={bankInfo.bankAccountNum} onChange={e => setBankInfo({...bankInfo, bankAccountNum: e.target.value})} />
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><FaHistory /> Payment & Invoice History</h3>
              ...
              {invoices.length > 0 ? invoices.map((inv: any) => (
                <tr key={inv.id} className="border-b hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 text-right">
                    <button onClick={() => setSelectedInvoice(inv)} className="p-2 hover:bg-gray-200 rounded-lg transition" title="Open invoice"><FaEye className="text-gray-600" /></button>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan={4} className="p-10 text-center text-gray-400 italic">No invoices found for your account.</td></tr>
              )}
            </tbody>
          </table>
        </div>
