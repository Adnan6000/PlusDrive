...
export default function Inbox() {
  ...
  const chatContainerRef = useRef<HTMLDivElement>(null); // To track scroll position
  ...
  // We track the previous number of messages to know if a NEW one arrived
  const prevMsgCount = useRef(0);

  useLayoutEffect(() => {
    // Only scroll if:
    // A. It's the first load (messages went from 0 to X)
    // B. A NEW message arrived (count increased)
    // C. We are NOT in selection mode (don't jump while selecting)
  ...
  const getRoleBadge = (role: string) => { /* ... Keep code ... */ 
    const isTeacher = role === 'ADMIN' || role === 'INSTRUCTOR';
    return isTeacher ? <span className="flex items-center gap-1 text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full uppercase tracking-wide"><FaChalkboardTeacher /> Instructor</span> : <span className="flex items-center gap-1 text-[10px] font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full uppercase tracking-wide"><FaUserGraduate /> Student</span>;
  };
  const getDayLabel = (dateString: string) => { /* ... Keep code ... */ 
  ...
  const startChatWithUser = async (userId: string) => { /* ... Keep code ... */
    try {
  ...
  const handleDeleteSelected = async () => {
      ...
      // Call delete for each (In a real app, you'd make a bulk-delete endpoint)
  ...
  // Standard Send Logic
  const handleSendMessage = async (e: any) => { /* ... Keep code ... */ 
    e.preventDefault();
    ...
    setNewMessage('');
    // Scroll will happen automatically due to useLayoutEffect
    try {
  ...
  // File & Location Logic (Same as before)
  const handleFileSelect = async (e: any) => { /* ... Keep code ... */ 
  ...
  const handleLocation = () => { /* ... Keep code ... */ 
  ...
  return (
    <div className="flex h-[80vh] md:h-[calc(100vh-140px)] bg-slate-50 border border-slate-200 rounded-2xl shadow-2xl overflow-hidden font-sans relative">
      <input type="file" ref={fileInputRef} onChange={handleFileSelect} className="hidden" aria-label="File Upload" title="File Upload" />
         ...
         <div className="p-5 bg-gradient-to-b from-white to-slate-50 border-b border-slate-100 z-10">
          ...
          {isInstructor && (
            <div className="relative group">
               ...
               <input type="text" placeholder="Search students..." className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 bg-slate-100 focus:bg-white focus:ring-2 focus:ring-blue-400 focus:outline-none transition-all shadow-inner" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}/>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {contacts.map(contact => (
            <div key={contact.id} onClick={() => setActiveChat(contact)} className={`group relative p-3 flex items-center gap-3 cursor-pointer rounded-xl transition-all duration-300 ease-out border ${activeChat?.id === contact.id ? 'bg-white border-blue-200 shadow-lg md:scale-[1.02] md:translate-x-1' : 'bg-transparent border-transparent hover:bg-white hover:border-slate-100 hover:shadow-md hover:-translate-y-0.5'}`}>
              <div className={`h-12 w-12 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-inner shrink-0 ${activeChat?.id === contact.id ? 'bg-[radial-gradient(circle_at_30%_30%,_#60a5fa,_#2563eb)] ring-2 ring-blue-100' : 'bg-[radial-gradient(circle_at_30%_30%,_#94a3b8,_#475569)]'}`}>{contact.name?.charAt(0) || '?'}</div>
              <div className="overflow-hidden flex-1">
                <div className="flex justify-between items-start"><p className={`font-bold text-sm truncate ${activeChat?.id === contact.id ? 'text-blue-800' : 'text-slate-700'}`}>{contact.name || contact.fullName}</p><span className="text-[10px] text-slate-400">{new Date(contact.date).toLocaleDateString(undefined, {month:'short', day:'numeric'})}</span></div>
      ...
      <div className={`w-full md:w-2/3 flex flex-col bg-slate-50 absolute inset-0 md:static z-30 transition-transform duration-300 bg-white ${activeChat ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}>
        ...
        {activeChat ? (
          <>
            ...
            <div className="p-4 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between shadow-sm z-10">
              <div className="flex items-center gap-4">
                  <button onClick={() => setActiveChat(null)} className="md:hidden p-2 -ml-2 text-slate-600 hover:text-blue-600 transition" aria-label="Back" title="Back"><FaArrowLeft /></button>
                  <div className="h-10 w-10 rounded-full bg-[radial-gradient(circle_at_30%_30%,_#4ade80,_#16a34a)] flex items-center justify-center text-white font-bold shadow-md shrink-0">{activeChat.name?.charAt(0) || '?'}</div>
                  <div>
              ...
              <div className="relative">
                 {isSelectionMode ? (
                     <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={handleDeleteSelected} className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200" aria-label="Delete selected messages" title="Delete selected messages"><FaTrash /></button>
                        <button type="button" onClick={() => { setIsSelectionMode(false); setSelectedMsgIds([]); }} className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200" aria-label="Cancel selection" title="Cancel selection"><FaTimes /></button>
                     </div>
                 ) : (
                     <button type="button" onClick={() => setShowChatMenu(!showChatMenu)} className="p-2 text-slate-500 hover:text-blue-600 transition rounded-full hover:bg-slate-100" aria-label="Chat options" title="Chat options">
                        <FaEllipsisV />
                     </button>
                 )}
                 ...
                 {/* DROPDOWN MENU */}
                 {showChatMenu && (
                    <div className="absolute right-0 top-12 bg-white shadow-2xl rounded-xl border border-slate-100 w-48 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                ...
                return (
                  <div key={msg.id} className="relative">
                    ...
                    <div className={`flex items-end gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'} group`}>
                      ...
                      {/* ✅ SELECTION CHECKBOX */}
                      {isSelectionMode && (
                          <div onClick={() => toggleSelection(msg.id)} className={`h-5 w-5 rounded border cursor-pointer flex items-center justify-center transition-all ${isSelected ? 'bg-blue-500 border-blue-500 text-white' : 'border-slate-300 bg-white'}`}>
                              {isSelected && <FaCheck size={10} />}
                          </div>
                      )}

                      <div className={`
                         relative max-w-[85%] md:max-w-[75%] p-4 rounded-2xl text-sm shadow-md transition-transform 
                         ${isDeleted ? 'bg-slate-100 border border-slate-200 text-slate-400 italic' : 
                           isMe ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-br-none shadow-blue-200' : 'bg-white text-slate-700 border border-slate-100 rounded-bl-none shadow-slate-200'}
                      `}>
                        {/* ATTACHMENTS */}
                        {!isDeleted && msg.attachmentType === 'IMAGE' && (
                            <div className="mb-2 rounded-lg overflow-hidden border border-white/20">
                                <img src={msg.attachmentUrl.startsWith('blob:') ? msg.attachmentUrl : `${BASE_URL}${msg.attachmentUrl}`} alt="attachment" className="w-full h-auto max-h-60 object-cover" />
                            </div>
                        )}
                        {!isDeleted && msg.attachmentType === 'DOCUMENT' && (
                            <div className="flex items-center gap-3 bg-black/10 p-3 rounded-lg mb-2">
                                <FaFileAlt className="text-2xl" />
                                <a href={msg.attachmentUrl.startsWith('blob:') ? msg.attachmentUrl : `${BASE_URL}${msg.attachmentUrl}`} target="_blank" rel="noreferrer" className="underline font-bold text-xs truncate max-w-[150px]">Download File</a>
                            </div>
                        )}

                        <p className="relative z-10 leading-relaxed break-words whitespace-pre-wrap">
                            {msg.content?.startsWith('https://www.google.com/maps?q=$$') ? (
                ...
                <div className="p-4 bg-white border-t border-slate-200 shadow-[0_-5px_20px_rgba(0,0,0,0.03)] z-20 relative">
                  ...
                  {showAttachments && (
                      <div className="absolute bottom-20 left-4 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 flex flex-col gap-2 animate-in slide-in-from-bottom-5 duration-200 z-50 w-48">
                          <button onClick={triggerFileUpload} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-slate-600 hover:text-blue-600 transition text-sm font-bold text-left"><div className="bg-blue-100 p-2 rounded-full text-blue-600"><FaImage /></div> Photo</button>
                          <button onClick={handleLocation} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-slate-600 hover:text-green-600 transition text-sm font-bold text-left"><div className="bg-green-100 p-2 rounded-full text-green-600"><FaMapMarkerAlt /></div> Location</button>
                          <button onClick={triggerFileUpload} className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-slate-600 hover:text-red-600 transition text-sm font-bold text-left"><div className="bg-red-100 p-2 rounded-full text-red-600"><FaFileAlt /></div> Document</button>
                      </div>
                  )}
                  <form onSubmit={handleSendMessage} className="flex gap-3 items-center">
                    <button type="button" onClick={() => setShowAttachments(!showAttachments)} className={`p-4 rounded-full transition-all duration-300 shadow-sm border border-slate-200 ${showAttachments ? 'bg-slate-800 text-white rotate-45' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`} aria-label="Add attachment" title="Add attachment"><FaPlus /></button>
                    <input type="text" placeholder="Type your message..." className="flex-1 p-4 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-50 focus:outline-none transition-all shadow-inner" value={newMessage} onChange={e => setNewMessage(e.target.value)} />
                    <button title='submit' type="submit" className="bg-gradient-to-b from-blue-500 to-blue-700 text-white p-4 rounded-xl hover:from-blue-400 hover:to-blue-600 transition-all shadow-[0_4px_0_rgb(30,58,138)] active:shadow-none active:translate-y-[4px]"><FaPaperPlane className="text-lg drop-shadow-md" /></button>
                  </form>
                </div>
            )}
          </>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-300 select-none bg-slate-50">
            <div className="h-32 w-32 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 shadow-inner flex items-center justify-center mb-6"><FaCommentDots className="text-6xl text-slate-300 drop-shadow-sm" /></div>
